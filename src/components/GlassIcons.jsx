const gradientMapping = {
  ...
  const GlassIcons = ({ items, className }) => {
    ...
    return (
      <div
        className={`grid gap-10 md:gap-20 grid-cols-3 md:grid-cols-6 mx-auto py-[3em] overflow-visible ${className || ""
          }`}
      >
        {items.map((item, index) => (
          ...
          >
            ...
            {/* Label */}
            <span className="absolute top-full font-right-light font-thin left-0 right-0 text-center whitespace-nowrap leading-[2] text-base opacity-0 transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] translate-y-0 group-hover:opacity-100 group-hover:[transform:translateY(20%)]">
              {item.label}
            </span>
          </button>
        ))}
